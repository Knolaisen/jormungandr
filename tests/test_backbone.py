...
def test_backbone_output_shape():
    ...
    assert features.shape[0] == batch_size, f"Expected batch size {batch_size}, got {features.shape[0]}"
    assert mask.shape[0] == batch_size, f"Expected batch size {batch_size}, got {mask.shape[0]}"